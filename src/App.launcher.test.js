...
test("publishes the 0.3.9 release markers", () => {
  assert.equal(packageJson.version, "0.3.9");
  assert.match(app, /worldbook-assets\/\$\{fileName\}\?v=0\.3\.9/);
  assert.match(app, /Ccat OS V0\.3\.9/);
  assert.match(styles, /worldbook-assets\/hero-worldbook-atlas\.png\?v=0\.3\.9/);
});
