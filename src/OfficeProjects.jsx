...
import { readWalletData } from './walletStore.js';
const money=n=>n.toLocaleString('zh-CN');
export function OfficeProjects({storage,onBack}){
 ...
 const run=async(refresh=false)=>{
  ...
  try{if(navigator.locks)await navigator.locks.request('ccat-office-projects',action);else action();}
 ...
 return <>
  ...
  <main className="ow-projects" aria-label="项目管理内容" aria-busy={busy}>
   ...
   {board&&<div className="ow-project-list">{board.projects.map((p,index)=><article className="ow-project-card" key={p.id}><div className="ow-project-card-top"><span className="ow-project-number">{String(index+1).padStart(2,'0')}</span><span className="ow-project-time"><Clock3 size={13}/>预计 {p.minutes} 分钟</span></div><h3>{p.name}</h3><p>{p.content}</p><div className="ow-project-reward"><span>项目金额</span><strong>¥ {money(p.amount)}</strong></div></article>)}</div>}
  </main>
 </>;
}
