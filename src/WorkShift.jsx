...
export function WorkShift({ career, world, people, remaining, onBegin, onScene, onFinish, onNextDay, onChat, paying, payError, compact = false, onActivity, onPartner }) {
  const [chosen, setChosen] = useState('');
  const [busy, setBusy] = useState('');
  const [error, setError] = useState('');
  const pending = useRef(null);
  const activeCareer = useRef('');
  activeCareer.current = `${career.payrollId}:${career.day}`;
  ...
  useEffect(() => { onPartner?.(partner?.id || ''); }, [partner?.id, onPartner]);
  useEffect(() => { setChosen(''); setError(''); setBusy(''); pending.current?.abort(); pending.current = null; }, [career.payrollId, career.day]);
  const speak = async (phase, snapshot = career, choice = '') => {
    ...
    try {
      ...
      if (activeCareer.current !== `${snapshot.payrollId}:${snapshot.day}`) return;
      if (!controller.signal.aborted) setError(`${err.message} 你仍然可以继续计时，到时收工领薪。`);
      else setError('回应已取消或超时。工作进度保留，可以重试，也可以继续工作。');
    } finally { clearTimeout(timeout); if (pending.current === controller) { pending.current = null; setBusy(''); } }
  };
  ...
  return <section className={`ws-shift ${compact ? 'ws-shift-compact' : ''}`} aria-label="今日角色工作">
    ...
    <article className="ws-paper ws-partner-card">
      {partner ? <>
        ...
        {!recorded && !busy && !project.delivered && <details className="ws-partner-choice" open={compact ? undefined : true}><summary>更换搭档</summary><label className="ws-label">选择搭档<select value={partner.id} onChange={e => setChosen(e.target.value)}>{people.map(p => <option value={p.id} key={p.id}>{p.name}</option>)}</select></label></details>}
    ...
    {project.accepted && (!compact || remaining <= 0 || project.salaryPaid) && <article className="ws-paper"><h2>{project.salaryPaid ? '今天的工资已到账' : remaining > 0 ? '工作正在推进' : '可以收工了'}</h2><p className="ws-muted">{project.salaryPaid ? '共同经历已留在职业档案，明天还会有新的事情。' : remaining > 0 ? `还剩 ${formatCountdown(remaining)}，可以和搭档互动，也可以切换到其他 APP。` : '不用补文件或检查，收工即可结算工资。'}</p>
