...
const worldbookAsset = (fileName) => `${import.meta.env.BASE_URL}worldbook-assets/${fileName}?v=0.2.88`;
const workMapAsset = (fileName) => `${import.meta.env.BASE_URL}work-map-assets/${fileName}?v=0.2.88`;
const workOutlineAsset = (themeId, placeType) => `${import.meta.env.BASE_URL}work-map-outlines/${themeId}-${placeType}.png?v=0.2.88`;
    ...
    color === CHROME_COLORS.white
      ...
      : color === CHROME_COLORS.me
        ...
        : color === CHROME_COLORS.worldbook
          ? "worldbook"
          : "default";
  const themeColor = chromeMode === "worldbook" ? "transparent" : color || CHROME_COLORS.home;
  document.querySelectorAll('meta[name="theme-color"]').forEach((meta) => {
    meta.setAttribute("content", themeColor);
  });
  document.documentElement.dataset.chromeColor = chromeMode;
  document.body.dataset.chromeColor = chromeMode;
  const background = chromeMode === "worldbook" ? WORLDBOOK_CHROME_BACKGROUND : color || CHROME_COLORS.home;
  const chromeBackgroundColor = chromeMode === "worldbook" ? "transparent" : color || CHROME_COLORS.home;
...
function SettingsScreen({ onOpen }) {
  return (
    <section className="screen-view settings-view">
      ...
      <p className="version-label">Ccat OS V0.2.88</p>
    </section>
  );
}
