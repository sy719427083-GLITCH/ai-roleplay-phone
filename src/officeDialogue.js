import { storyMemories, STORY_ACTIONS } from './officeStories.js';
...
function participantContext(storage,p) {
  ...
  return {id:p.id,name:p.name,position:officeRoleLabel(p.role),manager:p.managerId,currentTask:p.currentTask,coworkerMemories:storyMemories(storage,p.sourceKey),sharedMemories:officeMemories(storage,p.sourceKey),relationshipWithPlayer:officeRelationship(readOfficeEvents(storage).history,p.sourceKey),
...
export async function requestOfficeDialogue({storage,participants,topic,signal,autonomous=false,fetchImpl=fetch}) {
  ...
  const response=await fetchImpl(`${base}/chat/completions`,{method:'POST',signal,headers:{'Content-Type':'application/json',Authorization:`Bearer ${endpoint.apiKey.trim()}`},body:JSON.stringify({model,temperature:Number(endpoint.temperature??.7),max_tokens:1000,messages:[
    ...
    ...(autonomous?[{role:'system',content:'当前是自主办公室故事，观察者不会下命令。参与者自行协商下一步，避免向观察者提问或等待选择。根据人物性格、职位、当前工作和coworkerMemories生成自然的交流。JSON除messages外必须包含action，且只能为coffee（一起去茶水吧）、cooperate（到工位一起处理问题）、work（各自回工位工作）。台词与行动一致，只表达即将执行的决定，不宣称已完成。'}]:[]),
  ...
  const messages=parseOfficeDialogue(content,participants);
  if(!autonomous)return messages;
  let value;try{value=JSON.parse(content.trim().replace(/^```(?:json)?\s*/i,'').replace(/\s*```$/,''));}catch{throw Error('AI 行动格式不正确。');}
  if(!Object.hasOwn(STORY_ACTIONS,value.action))throw Error('AI 返回了不支持的行动。');
  return {messages,action:value.action};
}
