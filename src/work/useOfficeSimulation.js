...
export function releaseOfficeConversationPlan({ generatedPlan, occupants = [], projectContext = null, intervalKey = "", completedAt = Date.now() }) {
  ...
  return allocateOfficeActivities({ ...generatedPlan, id: `${generatedPlan.id}:after-chat:${completedAt}`, characters, conversation: null }, occupants, {
...
export function useOfficeSimulation({ occupants, simulation, dispatch, companyName = "office", projectContext = null, sceneRef, now, showNotice }) {
  ...
  const key = `${getOfficeIntervalKey(new Date(now))}:${simulation.mode}:${projectKey}:${occupants.map((item) => `${item.slotId}:${item.profile.id}`).join("|")}`;
  ...
  useEffect(() => {
    ...
    const createPlan = () => createLocalOfficePlan({ occupants, now: new Date(now), seed, projectContext, previousPlan: simulation.plan });
    let localPlan = deriveCurrentSimulation({ persisted: simulation, intervalKey, occupants, createPlan });
    localPlan = allocateOfficeActivities(localPlan, occupants, { projectContext, intervalKey, now });
    ...
    generateAiOfficePlan({ apiState, context: buildOfficeAiContext({ occupants, now, endsAt: localPlan.endsAt, projectContext }) })
      .then((aiPlan) => {
        if (cancelled) return;
        const safePlan = allocateOfficeActivities(aiPlan, occupants, { projectContext, intervalKey, now });
        setPlan(safePlan);
  ...
  useEffect(() => {
    ...
    const resume = () => {
      ...
      const intervalKey = getOfficeIntervalKey(new Date(resumedAt));
      const freshPlan = allocateOfficeActivities(createLocalOfficePlan({ occupants, now: new Date(resumedAt), seed, projectContext, previousPlan: plan }), occupants, { projectContext, intervalKey, now: resumedAt });
  ...
  useEffect(() => {
    ...
    occupants.forEach((occupant) => {
      ...
      setCharacterStates((current) => ({
        ...current,
        [profileId]: current[profileId] || { node: getOfficePoint(`${occupant.slotId}-home`), moving: false, facing: "right", durationMs: 0, activity: "idle", label: "待命中" },
      }));
  ...
  const finishConversation = () => {
    ...
    const releasedPlan = releaseOfficeConversationPlan({ generatedPlan: generated, occupants, projectContext, intervalKey, completedAt });
    setConversationReadyId("");
  ...
  const { activeConversation, currentTurn } = useOfficeConversation({
    ...
    projectContext: projectContext?.project?.name || "",
    now,
  ...
  const commandMe = (target) => {
    ...
    const next = allocateOfficeActivities(
      ...
      occupants,
      { projectContext, intervalKey: getOfficeIntervalKey(new Date()), now: Date.now() },
    );
