...
const setup=()=>{const s=memory();return {s,board:openProjectBoard(s,now)};};
test('accept saves real start/deadline and immutable reward; three accepts per day',()=>{
 const {s,board}=setup();for(let i=0;i<3;i++)acceptOfficeProject(s,board.projects[i].id,now+i);
...
test('double click cannot accept same project twice and stale offers are rejected',()=>{
 const {s,board}=setup();acceptOfficeProject(s,board.projects[0].id,now);assert.throws(()=>acceptOfficeProject(s,board.projects[0].id,now),/已接取/);assert.throws(()=>acceptOfficeProject(s,'missing',now),/刷新/);assert.equal(readOfficeJobs(s).length,1);
});
test('refresh does not lose active work; timer reflects real elapsed time after reload',()=>{
 const {s,board}=setup();acceptOfficeProject(s,board.projects[0].id,now);const job=readOfficeJobs(s)[0];refreshProjectBoard(s,now);assert.deepEqual(readOfficeJobs(s)[0],job);assert.equal(remainingJobMs(readOfficeJobs(s)[0],now+60000),job.minutes*60000-60000);assert.equal(remainingJobMs(job,job.endsAt+123),0);
});
...
test('next local day restores allowance; completion does not replenish same-day quota',()=>{
 const {s,board}=setup();for(let i=0;i<3;i++)acceptOfficeProject(s,board.projects[i].id,now);settleOfficeJobs(s,now+86400000);assert.equal(availableAccepts(readOfficeJobs(s),now),0);assert.equal(availableAccepts(readOfficeJobs(s),now+86400000),3);acceptOfficeProject(s,board.projects[0].id,now+86400000);assert.equal(readOfficeJobs(s).length,4);
});
