...
- Added a fine local-motion coverage fingerprint so legitimate small limb movement survives validation without weakening copied-frame or single-pixel defenses.
...
- Fourth review-focused run: 23 tests, 22 passed, 1 failed. A real 384px fixture with an 8px-wide arm moving smoothly by 16px reproduced the false rejection, while real copied frames and per-frame single-pixel changes already remained rejected.
...
- Fourth review-focused animation inspection tests: 23 passed, 0 failed.
- Fourth review-focused Work and all Pixi tests: 72 passed, 0 failed.
...
- Full repository suite: 225 passed, 0 failed.
...
- The real inspector combines an 8x8 alpha/luminance fingerprint with a 48x48 local alpha-motion fingerprint for every frame. At 384px, local diversity needs roughly 15 changed pixels, allowing an 8px limb to move while rejecting copied frames and single-pixel noise. Locomotion still needs at least four perceptually distinct frames; idle and action clips need at least two.
...
- Perceptual validation deliberately rejects motion below the local coverage floor. At 384px this is approximately 15 changed pixels, with slight rounding variation at other supported frame sizes.
