...
`node --test src/work/officeState.test.js src/work/WorkAppScreen.test.js src/work/officeWorld.test.js` failed in four places before the fix:

- no production frame helper applied `separateActors`, so two legal crossing routes rendered at the same coordinate;
- `officeWorld.js` did not expose its authoritative route validator;
- `START_WORLD_ROUTE` accepted direct cross-scene and collider-crossing routes that the sampler rejected;
- forged `CROSS_SCENE_DOOR` and premature `FINISH_RETURN` actions changed state before route progress allowed them.

### GREEN

- focused Task 13 state/screen/world tests: 38 passed;
- scheduler and reservation regressions: 20 passed;
- full `npm test`: 191 passed;
- `npm run build`: passed;
- `git diff --check`: clean.

### Decisions

- `isValidWorldRoute` is now the shared route authority used by both the sampler and reducer input validation.
- route state stores its speed; reducer movement transitions re-sample the active route at the supplied clock and accept only matching scene, position, segment, and completion state.
- `samplePhysicalWorldFrame` keeps raw route samples for reducer transitions and separately collision-resolves render samples before Pixi receives them. Previous rendered positions provide deterministic waiting without moving exact activity anchors.
- paired-door traversal retains the original route start time and remaining distance. Return completion requires the authoritative sample to be done at the exact home anchor.
